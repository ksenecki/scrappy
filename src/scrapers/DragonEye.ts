...
class ShopDragonEye {
  async dragonEye(pageNumber: number) {
    ...
    await page.goto(BASE_URL + `gry-planszowe-c-15.html?page=${pageNumber}&sort=1a`);


    const products = await page.$$eval('.listing .boxProdSmall', (productArticles) => {
      return productArticles.map((product) => {
        const title = product.querySelectorAll('.center .nazwa')[0].textContent;
